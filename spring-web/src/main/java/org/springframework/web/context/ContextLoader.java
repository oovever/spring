...
public class ContextLoader {
	 ...
	 */
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) { // 初始化 webApplicationContext
//		初始化完成的 webApplicationContext 会被保存到  servletContext 的属性中，key 为 ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE
		if (servletContext.getAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE) != null) { // 已经初始化则报错,防止多次初始化
		...
		try {
			...
			if (this.context == null) { // xml在这里创建
				this.context = createWebApplicationContext(servletContext); // 创建WebApplicationContext
			}
			if (this.context instanceof ConfigurableWebApplicationContext) { // 注解方式走这里
			...
			servletContext.setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, this.context); // 保存父容器
			...
			if (ccl == ContextLoader.class.getClassLoader()) {
				...
				currentContextPerThread.put(ccl, this.context); // 保存类加载器与实例变量
			}
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		if (contextClassName != null) {
			...
			try {
				return ClassUtils.forName(contextClassName, ContextLoader.class.getClassLoader()); // 通过反射获取实例
			}
			catch (ClassNotFoundException ex) {
				throw new ApplicationContextException(
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		wac.setServletContext(sc); // 设置servletContext到spring的上下文
		String configLocationParam = sc.getInitParameter(CONFIG_LOCATION_PARAM); // 获取xml配置的contextConfigLocation参数
