...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		Object handler = getHandlerInternal(request); // 根据请求获取handler，子类实现
		if (handler == null) { // handler为空获取默认的Handler
		...
		if (handler instanceof String) { // string类型的handler则为beanName 通过getBean获取实例
		...
		HandlerExecutionChain executionChain = getHandlerExecutionChain(handler, request); // 生成拦截器链
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		HandlerExecutionChain chain = (handler instanceof HandlerExecutionChain ?
				(HandlerExecutionChain) handler : new HandlerExecutionChain(handler)); // 构建HandlerExecutionChain执行链

		for (HandlerInterceptor interceptor : this.adaptedInterceptors) { // 遍历所有拦截器
			if (interceptor instanceof MappedInterceptor) {
				MappedInterceptor mappedInterceptor = (MappedInterceptor) interceptor;
				if (mappedInterceptor.matches(request)) {
					chain.addInterceptor(mappedInterceptor.getInterceptor()); // 添加过滤器
				}
			}
			else {
