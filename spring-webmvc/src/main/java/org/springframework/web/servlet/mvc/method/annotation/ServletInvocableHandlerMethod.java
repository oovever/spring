...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
	...
	public void invokeAndHandle(ServletWebRequest webRequest, ModelAndViewContainer mavContainer,
		...
		Object returnValue = invokeForRequest(webRequest, mavContainer, providedArgs); // 反射调用目标方法，获取返回值
		setResponseStatus(webRequest);

		if (returnValue == null) { // 请求处理完成，设置requestHandled属性
		...
		else if (StringUtils.hasText(getResponseStatusReason())) {  // 请求失败
			mavContainer.setRequestHandled(true);
			return;
		}
		...
		try {
			this.returnValueHandlers.handleReturnValue(
					returnValue, getReturnValueType(returnValue), mavContainer, webRequest); // 处理返回值，不同返回值，使用不同的返回值解析器
		}
		catch (Exception ex) {
