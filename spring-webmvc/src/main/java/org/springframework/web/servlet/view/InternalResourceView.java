...
public class InternalResourceView extends AbstractUrlBasedView {
	...
	protected void renderMergedOutputModel(
		...
		exposeModelAsRequestAttributes(model, request); // 将model的值设置的request的attribute中

		// Expose helpers as request attributes, if any. 设置国际化资源
		exposeHelpers(request);

		// Determine the path for the request dispatcher.
		String dispatcherPath = prepareForRendering(request, response); // 防止死循环请求，请求的/a 响应的是a

		// Obtain a RequestDispatcher for the target resource (typically a JSP).
		RequestDispatcher rd = getRequestDispatcher(request, dispatcherPath); //通过请求和地址获取RequestDispatcher
		if (rd == null) {
