...
public abstract class AbstractDispatcherServletInitializer extends AbstractContextLoaderInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		super.onStartup(servletContext); // 执行父类的onStartup
	...
	protected void registerDispatcherServlet(ServletContext servletContext) {
		...
		WebApplicationContext servletAppContext = createServletApplicationContext(); // 创建servlet子容器
		Assert.notNull(servletAppContext, "createServletApplicationContext() must not return null");

		FrameworkServlet dispatcherServlet = createDispatcherServlet(servletAppContext); // 创建dispatcherServlet
		...
		registration.setLoadOnStartup(1); // 设置启动时加载
		registration.addMapping(getServletMappings()); // 设置映射 getServletMappings子类实现
		registration.setAsyncSupported(isAsyncSupported());

		Filter[] filters = getServletFilters(); // 过滤器处理,子类实现
