...
class WebMvcConfigurerComposite implements WebMvcConfigurer {
	...
	private final List<WebMvcConfigurer> delegates = new ArrayList<>(); // 持有配置类的对象
	...
	public void configurePathMatch(PathMatchConfigurer configurer) {
		for (WebMvcConfigurer delegate : this.delegates) {
			delegate.configurePathMatch(configurer); // 配置注册给所有对象
		}
	}
	...
	public void addInterceptors(InterceptorRegistry registry) {
		for (WebMvcConfigurer delegate : this.delegates) { // 行每一个WebMvcConfigurer配置类的addInterceptors(registry)方法， 将用户自定义的拦截器注册到拦截器注册中心中
	...
	public Validator getValidator() {
		...
		for (WebMvcConfigurer configurer : this.delegates) {
			Validator validator = configurer.getValidator(); // 只允许一个配置重写方法返回Validator
			if (validator != null) {
				if (selected != null) {
	...
	@Override
	@Nullable
	public MessageCodesResolver getMessageCodesResolver() { // MessageCodesResolver是唯一的，只允许一个配置重写返回MessageCodesResolver
