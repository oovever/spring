...
public abstract class AbstractAnnotationConfigDispatcherServletInitializer
	...
	protected WebApplicationContext createRootApplicationContext() {
		Class<?>[] configClasses = getRootConfigClasses(); // getRootConfigClasses自定义类配置类
	...
	protected WebApplicationContext createServletApplicationContext() {
		AnnotationConfigWebApplicationContext context = new AnnotationConfigWebApplicationContext(); // 创建spring容器
		Class<?>[] configClasses = getServletConfigClasses(); // getServletConfigClasses自定义的实现，配置的子容器配置类
		if (!ObjectUtils.isEmpty(configClasses)) {
			context.register(configClasses); // configClasses注册到spring容器中
		}
		return context;
	}
