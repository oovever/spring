...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext initWebApplicationContext() {
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext()); // 获取WebApplicationContext 用于构建父容器
		WebApplicationContext wac = null;

		if (this.webApplicationContext != null) { // webApplicationContext 构造函数中注入，不为空直接使用，初始化DispatcherServlet 会传入子容器
			// A context instance was injected at construction time -> use it
			wac = this.webApplicationContext; // 获取子容器
			if (wac instanceof ConfigurableWebApplicationContext) {
				ConfigurableWebApplicationContext cwac = (ConfigurableWebApplicationContext) wac;
				if (!cwac.isActive()) {
					// The context has not yet been refreshed -> provide services such as
					// setting the parent context, setting the application context id, etc
					if (cwac.getParent() == null) { // 如果没有设置父容器
						// The context instance was injected without an explicit parent -> set
						// the root application context (if any; may be null) as the parent
						cwac.setParent(rootContext); // 设置父容器,父子容器
					}
					configureAndRefreshWebApplicationContext(cwac); // 配置并且加载子容器
				}
			}
		}
		if (wac == null) {
			...
			wac = findWebApplicationContext(); // 根据 contextAttribute 属性加载 WebApplicationContext,即通过 web.xml 文件中配置的servlet 参数 contextAttribute 来查找 ServerContext 中对应的属性
		}
		if (wac == null) {
			// No context instance is defined for this servlet -> create a local one
			wac = createWebApplicationContext(rootContext); // 自己尝试创建WebApplicationContext
		}
	...
	protected WebApplicationContext findWebApplicationContext() {
		String attrName = getContextAttribute(); //ContextAttribute 初始化 DispatcherServlet时通过 setContextAttribute 进行设置,默认是Null；
		if (attrName == null) {
			return null;
		}
		WebApplicationContext wac =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext(), attrName); // 使用ContextAttribute作为key通过ServletContext中获取WebApplicationContext
		if (wac == null) {
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		Class<?> contextClass = getContextClass(); // 获取Servlet初始化参数contextClass，WebApplicationContext的具体类型，默认是XmlWebApplicationContext
		...
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass); // 通过反射方式创建
		...
		configureAndRefreshWebApplicationContext(wac); // 配置ConfigurableWebApplicationContext实例并且刷新 wac.refresh()
...
//		设置属性信息
		...
		wac.addApplicationListener(new SourceFilteringListener(wac, new ContextRefreshListener())); //添加监听器，委托设计模式
		...
		wac.refresh(); // 加载容器 AbstractApplicationContext#refresh
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		HttpMethod httpMethod = HttpMethod.resolve(request.getMethod()); // 解析请求方式
		if (httpMethod == HttpMethod.PATCH || httpMethod == null) { // patch直接处理请求
...
// 		处理post请求
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		LocaleContext previousLocaleContext = LocaleContextHolder.getLocaleContext(); // 获取当前线程的LocaleContext
		LocaleContext localeContext = buildLocaleContext(request); // 根据当前请求创建对应的LocaleContext后绑定到当前线程上

		RequestAttributes previousAttributes = RequestContextHolder.getRequestAttributes(); // 获取当前线程的RequestAttributes
		ServletRequestAttributes requestAttributes = buildRequestAttributes(request, response, previousAttributes);
//		根据当前请求创建对应的ServletRequestAttributes后绑定到当前线程上
		...
		try {
			doService(request, response); // doService处理
		}
			...
			resetContextHolders(request, previousLocaleContext, previousAttributes); // 请求结束后恢复线程原始状态
			if (requestAttributes != null) {
				requestAttributes.requestCompleted();
			}
			logResult(request, response, failureCause, asyncManager);
			publishRequestHandledEvent(request, response, startTime, failureCause); // 发送请求结束通知事件 成功/失败
		}
	}
