...
public class AnnotationAwareAspectJAutoProxyCreator extends AspectJAwareAdvisorAutoProxyCreator {
	...
	protected List<Advisor> findCandidateAdvisors() {
		...
		List<Advisor> advisors = super.findCandidateAdvisors(); // 寻找所有advisor类型的bean对象
		// BeanFactoryUtils.beanNamesForTypeIncludingAncestors(
		//					this.beanFactory, Advisor.class, true, false) bean工厂中找所有Advisor类型的Bean
		// Build Advisors for all AspectJ aspects in the bean factory.
		if (this.aspectJAdvisorsBuilder != null) {
			advisors.addAll(this.aspectJAdvisorsBuilder.buildAspectJAdvisors()); // 切面解析得到Advisor对象@Aspect注解中定义的bean
		}
		return advisors;
	}
