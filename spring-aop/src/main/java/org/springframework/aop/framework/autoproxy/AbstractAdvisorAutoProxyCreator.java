...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	protected Object[] getAdvicesAndAdvisorsForBean(
		...
		List<Advisor> advisors = findEligibleAdvisors(beanClass, beanName); // 寻找匹配的advisor
		if (advisors.isEmpty()) { // 没有匹配的直接返回
			return DO_NOT_PROXY;
		}
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		List<Advisor> candidateAdvisors = findCandidateAdvisors(); // 寻找所有的advisor
		List<Advisor> eligibleAdvisors = findAdvisorsThatCanApply(candidateAdvisors, beanClass, beanName); // 进行筛选,找出匹配当前bean的
		extendAdvisors(eligibleAdvisors); // 扩展子类实现
		if (!eligibleAdvisors.isEmpty()) {
			eligibleAdvisors = sortAdvisors(eligibleAdvisors); // 对advisor排序 @Order注解或者@Ordered接口
		}
	...
	protected List<Advisor> findAdvisorsThatCanApply(
		...
		try {
			return AopUtils.findAdvisorsThatCanApply(candidateAdvisors, beanClass); // 筛选
		}
		finally {
