...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	...
	public Object proceed() throws Throwable {
		...
		if (this.currentInterceptorIndex == this.interceptorsAndDynamicMethodMatchers.size() - 1) { // 所有拦截器执行完成
			return invokeJoinpoint(); // 执行真正的方法
		}
//		currentInterceptorIndex 初始为-1
		...
		if (interceptorOrInterceptionAdvice instanceof InterceptorAndDynamicMethodMatcher) {
			...
			InterceptorAndDynamicMethodMatcher dm =
					(InterceptorAndDynamicMethodMatcher) interceptorOrInterceptionAdvice; // 强制转化拦截器为InterceptorAndDynamicMethodMatcher
			Class<?> targetClass = (this.targetClass != null ? this.targetClass : this.method.getDeclaringClass()); // 获取目标类
			if (dm.methodMatcher.matches(this.method, targetClass, this.arguments)) { // 根据方法参数匹配执行
				return dm.interceptor.invoke(this);
			}
			else {
				// Dynamic matching failed.
				// Skip this interceptor and invoke the next in the chain.
				return proceed(); // 递归调用下一个过滤器
			}
		}
		else {
			// It's an interceptor, so we just invoke it: The pointcut will have
			// been evaluated statically before this object was constructed.
			return ((MethodInterceptor) interceptorOrInterceptionAdvice).invoke(this); // 调用MethodBeforeAdviceInterceptor、MethodBeforeAdviceInterceptor、ThrowsAdviceAdapter的invoke方法
		}
	}
