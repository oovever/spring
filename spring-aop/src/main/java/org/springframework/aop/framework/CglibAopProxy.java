...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			Class<?> rootClass = this.advised.getTargetClass(); // 获取被代理的类
			Assert.state(rootClass != null, "Target class must be available for creating a CGLIB proxy");

			Class<?> proxySuperClass = rootClass;
			if (rootClass.getName().contains(ClassUtils.CGLIB_CLASS_SEPARATOR)) { // 如果被代理的类是cglib生成的代理类  如果当前类名中包含 “$$” 则被认定为是 cglib 类。
				proxySuperClass = rootClass.getSuperclass(); // 获取真实类
				Class<?>[] additionalInterfaces = rootClass.getInterfaces(); // 获取被代理类实现的接口
				for (Class<?> additionalInterface : additionalInterfaces) {
					this.advised.addInterface(additionalInterface);
				}
			}

			// Validate the class, writing log messages as necessary. 校验方法合法性，
			validateClassIfNecessary(proxySuperClass, classLoader);

			// Configure CGLIB Enhancer...
			Enhancer enhancer = createEnhancer(); // 创建Enhancer
			if (classLoader != null) {
			...
			Callback[] callbacks = getCallbacks(rootClass); // 获取与被代理类匹配的advisor
			...
			enhancer.setCallbackFilter(new ProxyCallbackFilter(
					this.advised.getConfigurationOnlyCopy(), this.fixedInterceptorMap, this.fixedInterceptorOffset)); // 设置CallbackFilter，CallbackFilter内部会判断被代理对象中的方法最终会被callbacks列表中的那个Callback来处理
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		boolean exposeProxy = this.advised.isExposeProxy(); // 是否暴露代理类
		boolean isFrozen = this.advised.isFrozen(); // 是否被冻结
		boolean isStatic = this.advised.getTargetSource().isStatic(); // 被代理的目标对象是否是动态的（是否是单例的）

		// Choose an "aop" interceptor (used for AOP calls). 构建AOP拦截器 当方法上有需要执行的拦截器的时候，会用这个来处理
		Callback aopInterceptor = new DynamicAdvisedInterceptor(this.advised);

		// Choose a "straight to target" interceptor. (used for calls that are
		// unadvised but can return this). May be required to expose the proxy.
		Callback targetInterceptor; // 方法上没有需要执行的拦截器的时候，会使用targetInterceptor来处理，内部会通过反射直接调用目标对象的方法
		if (exposeProxy) { // 根据是否暴露代理类和是否静态代理类生成不同的拦截器
		...
		Callback targetDispatcher = (isStatic ?
				new StaticDispatcher(this.advised.getTargetSource().getTarget()) : new SerializableNoOp()); // targetDispatcher会直接调用目标方法
		...
		if (isStatic && isFrozen) { //  如果被代理的对象是单例的 && 配置是冻结的，此时会进行优化,配置冻结的情况下，生成好的代理中通知是无法修改的，所以可以提前将每个方法对应的拦截器链找到给缓存起来,调用方法的时候，就直接从缓存中可以拿到方法对应的缓存信息，效率会高一些
			...
			for (int x = 0; x < methods.length; x++) {
				...
				this.fixedInterceptorMap.put(method, x); // 获取每个方法的调用链，然后给缓存在fixedInterceptorMap中
			}
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				if (this.advised.exposeProxy) { // 判断是否暴露代理类
					// Make invocation available if necessary.
					oldProxy = AopContext.setCurrentProxy(proxy);
					setProxyContext = true;
				}
				// Get as late as possible to minimize the time we "own" the target, in case it comes from a pool...
				target = targetSource.getTarget(); // 获取代理的目标对象
				Class<?> targetClass = (target != null ? target.getClass() : null);
				List<Object> chain = this.advised.getInterceptorsAndDynamicInterceptionAdvice(method, targetClass); // 获取当前方法的代理链，将AspectJMethodBeforeAdvice、AspectJAfterAdvice、AspectJAfterReturningAdvice 转换成合适的类型(InterceptorAndDynamicMethodMatcher)
				Object retVal;
				...
				if (chain.isEmpty() && CglibMethodInvocation.isMethodProxyCompatible(method)) {
					...
					Object[] argsToUse = AopProxyUtils.adaptArgumentsIfNecessary(method, args); // 获取方法参数，直接调用目标对象的方法
					try {
					...
					retVal = new CglibMethodInvocation(proxy, target, method, args, targetClass, chain, methodProxy).proceed(); // 创建一个方法调用器（包含了代理对象、目标对象、调用的方法、参数、目标类型、方法拦截器链）
//					最终调用的还是ReflectiveMethodInvocation.proceed
				}
				retVal = processReturnType(proxy, target, method, retVal); // 返回值类型不同，做不同处理
				return retVal;
			}
			finally {
