...
public class HandlerMethodReturnValueHandlerComposite implements HandlerMethodReturnValueHandler {
	...
	public Mono<Void> handleReturnValue(@Nullable Object returnValue, MethodParameter returnType, Message<?> message) {
		HandlerMethodReturnValueHandler handler = getReturnValueHandler(returnType); // 根据返回值类型，获取返回值处理器
		if (handler == null) {
		...
		return handler.handleReturnValue(returnValue, returnType, message); // 处理返回值
	}
