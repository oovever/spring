...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		TransactionAttributeSource tas = getTransactionAttributeSource(); // 获取事务的targetSource;transactionAttributeSource = AnnotationTransactionAttributeSource
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null); // 获取@Transactional注解中的值
		final TransactionManager tm = determineTransactionManager(txAttr); // 获取事务管理器TransactionManager bean对象
//		响应式事务处理
		...
		PlatformTransactionManager ptm = asPlatformTransactionManager(tm); // 判断是否	PlatformTransactionManager类型，如果不是则抛出异常
		final String joinpointIdentification = methodIdentification(method, targetClass, txAttr); // 构造方法唯一标识( 全路径了类名.方法)
//		获取方法名称当做事务的名称
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) { // 声明式事务处理
			// Standard transaction demarcation with getTransaction and commit/rollback calls.
			TransactionInfo txInfo = createTransactionIfNecessary(ptm, txAttr, joinpointIdentification); // 事务传播机制的实现，如果有必要才创建事务

			Object retVal;
			try {
				// This is an around advice: Invoke the next interceptor in the chain.
				// This will normally result in a target object being invoked.
				retVal = invocation.proceedWithInvocation(); // 执行代理方法
			}
			catch (Throwable ex) {
				...
				cleanupTransactionInfo(txInfo); // 提交之前清除事务信息
			}
		...
		else { // 响应式事务CallbackPreferringPlatformTransactionManager处理
			Object result;
			...
			try {
				result = ((CallbackPreferringPlatformTransactionManager) ptm).execute(txAttr, status -> {
					TransactionInfo txInfo = prepareTransactionInfo(ptm, txAttr, joinpointIdentification, status); // 准备事务信息
					try {
						Object retVal = invocation.proceedWithInvocation(); // 执行事务增强
						if (retVal != null && vavrPresent && VavrDelegate.isVavrTry(retVal)) {
							// Set rollback-only in case of Vavr failure matching our rollback rules...
							retVal = VavrDelegate.evaluateTryFailure(retVal, txAttr, status);
						}
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null && txAttr.getName() == null) { // 事务名称没有指定，使用唯一标识设置事务名称
			txAttr = new DelegatingTransactionAttribute(txAttr) {
				@Override
				public String getName() {
					return joinpointIdentification; // 构造方法唯一标识(全路径了类名.方法)
				}
			};
		}
	...
	protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		...
		TransactionInfo txInfo = new TransactionInfo(tm, txAttr, joinpointIdentification); // 封装TransactionInfo对象
		if (txAttr != null) {
		...
		txInfo.bindToThread(); // 绑定事务到当前线程上
		return txInfo;
	}
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.getTransactionStatus() != null) { // 当前线程中是否存在事务
			if (logger.isTraceEnabled()) {
				logger.trace("Completing transaction for [" + txInfo.getJoinpointIdentification() +
						"] after exception: " + ex);
			}
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) { // 是否触发回滚操作，RuntimeException 或 error类型 rollbackOn return (ex instanceof RuntimeException || ex instanceof Error);
				try {
					txInfo.getTransactionManager().rollback(txInfo.getTransactionStatus()); // 执行回滚
				}
				catch (TransactionSystemException ex2) {
				...
				try {
					txInfo.getTransactionManager().commit(txInfo.getTransactionStatus()); // 不满足回滚条件，继续提交
				}
				catch (TransactionSystemException ex2) {
