...
public class AnnotationTransactionAttributeSource extends AbstractFallbackTransactionAttributeSource
	...
	public boolean isCandidateClass(Class<?> targetClass) {
		for (TransactionAnnotationParser parser : this.annotationParsers) {
			if (parser.isCandidateClass(targetClass)) { //SpringTransactionAnnotationParser#isCandidateClass 中判断了目标类是否存在Transactional 注解
				return true;
			}
		}
		return false;
	...
	protected TransactionAttribute determineTransactionAttribute(AnnotatedElement element) {
		for (TransactionAnnotationParser parser : this.annotationParsers) {
			TransactionAttribute attr = parser.parseTransactionAnnotation(element); // SpringTransactionAnnotationParser解析
			if (attr != null) {
				return attr;
			}
		}
