...
public class MethodInvoker {
	...
	public static int getTypeDifferenceWeight(Class<?>[] paramTypes, Object[] args) {
		...
		for (int i = 0; i < paramTypes.length; i++) {
			if (!ClassUtils.isAssignableValue(paramTypes[i], args[i])) { // 不是当前类型，返回最大值，表示不匹配
				return Integer.MAX_VALUE;
			}
			if (args[i] != null) {
				Class<?> paramType = paramTypes[i]; // 类型的值
				Class<?> superClass = args[i].getClass().getSuperclass(); // 父类
				while (superClass != null) {
					if (paramType.equals(superClass)) { // 父类等于当前类型加2分
						result = result + 2;
						superClass = null;
					}
					else if (ClassUtils.isAssignable(paramType, superClass)) { //值的父类是当前类型的子类加2分
						result = result + 2;
				...
				if (paramType.isInterface()) { // 值的类型是接口加1分
					result = result + 1;
				}
			}
		}
