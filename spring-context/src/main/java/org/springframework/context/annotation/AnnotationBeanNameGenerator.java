...
public class AnnotationBeanNameGenerator implements BeanNameGenerator {
	...
	public String generateBeanName(BeanDefinition definition, BeanDefinitionRegistry registry) {
		if (definition instanceof AnnotatedBeanDefinition) {
			String beanName = determineBeanNameFromAnnotation((AnnotatedBeanDefinition) definition); // 判断component注解上是否设置beanName
			if (StringUtils.hasText(beanName)) {
				// Explicit bean name found.
				return beanName;
			}
		}
		// Fallback: generate a unique default bean name. 生成默认惟一的beanName
	...
	protected String buildDefaultBeanName(BeanDefinition definition) {
		...
		return Introspector.decapitalize(shortClassName); // 生成BeanName 第一 第二个字符都是大写 直接返回类名称
	}
