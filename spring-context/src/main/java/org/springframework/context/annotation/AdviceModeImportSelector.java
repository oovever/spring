...
public abstract class AdviceModeImportSelector<A extends Annotation> implements ImportSelector {
	...
	public final String[] selectImports(AnnotationMetadata importingClassMetadata) {
		Class<?> annType = GenericTypeResolver.resolveTypeArgument(getClass(), AdviceModeImportSelector.class); // 获取注解类型,EnableTransactionManagement
		Assert.state(annType != null, "Unresolvable type argument for AdviceModeImportSelector");

		AnnotationAttributes attributes = AnnotationConfigUtils.attributesFor(importingClassMetadata, annType); // 解析注解参数
		if (attributes == null) {
		...
		AdviceMode adviceMode = attributes.getEnum(getAdviceModeAttributeName()); // 获取mode属性，默认是PROXY
		String[] imports = selectImports(adviceMode); // 调用TransactionManagementConfigurationSelector的selectImports
		if (imports == null) {
