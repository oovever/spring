...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		txObject.setSavepointAllowed(isNestedTransactionAllowed()); // // 是否允许设置保存点,决定是否允许嵌套事务的存在
		ConnectionHolder conHolder =
				(ConnectionHolder) TransactionSynchronizationManager.getResource(obtainDataSource()); // 已有数据库链接，使用原有的连接
		txObject.setConnectionHolder(conHolder, false); //false非新建连接
		return txObject;
	}
	...
	protected boolean isExistingTransaction(Object transaction) {
		...
		return (txObject.hasConnectionHolder() && txObject.getConnectionHolder().isTransactionActive()); // 存在连接，且其 transactionActive 属性不为true
	}

	@Override
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		DataSourceTransactionObject txObject = (DataSourceTransactionObject) transaction;
		Connection con = null; // 获取数据库链接对象

		try {
			if (!txObject.hasConnectionHolder() ||
					txObject.getConnectionHolder().isSynchronizedWithTransaction()) { // 当前线程中数据库连接不存在或者事务同步为true时，需要重新获取数据库连接进行同步
				Connection newCon = obtainDataSource().getConnection(); // 获取数据库链接对象
				if (logger.isDebugEnabled()) {
					logger.debug("Acquired Connection [" + newCon + "] for JDBC transaction");
				}
				txObject.setConnectionHolder(new ConnectionHolder(newCon), true); // 封装对象 设置到tConnectionHolder
			}

			txObject.getConnectionHolder().setSynchronizedWithTransaction(true); // 同步标识设置为true
			...
			if (con.getAutoCommit()) { // 数据库链接autocommit设置为false
			...
			prepareTransactionalConnection(con, definition); // 准备事务连接 执行SET TRANSACTION READ ONLY
			...
			if (txObject.isNewConnectionHolder()) { // 新连接，绑定数据库连接到当前线程
	...
	protected void doSetRollbackOnly(DefaultTransactionStatus status) {
		...
		txObject.setRollbackOnly(); // 设置回滚标记位
	}
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (txObject.isNewConnectionHolder()) {
			TransactionSynchronizationManager.unbindResource(obtainDataSource()); // 数据库链接与线程解绑
		}

		// Reset connection.
		Connection con = txObject.getConnectionHolder().getConnection(); // 释放链接
		try {
		...
		if (txObject.isNewConnectionHolder()) {
			...
			DataSourceUtils.releaseConnection(con, this.dataSource); // 释放数据库链接
		}
	...
	protected void prepareTransactionalConnection(Connection con, TransactionDefinition definition)
		...
		if (isEnforceReadOnly() && definition.isReadOnly()) {
			try (Statement stmt = con.createStatement()) {
				stmt.executeUpdate("SET TRANSACTION READ ONLY"); // 设置事务为只读
			}
		}
	}
