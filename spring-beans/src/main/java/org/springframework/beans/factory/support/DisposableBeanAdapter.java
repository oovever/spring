...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	private static String inferDestroyMethodIfNecessary(Object bean, RootBeanDefinition beanDefinition) {
		...
		if (destroyMethodName == null) {
			...
			boolean autoCloseable = (bean instanceof AutoCloseable); // 实现AutoCloseable接口
			if (AbstractBeanDefinition.INFER_METHOD.equals(destroyMethodName) || // 设置(inferred)方法
				...
				if (!(bean instanceof DisposableBean)) { // 是否实现DisposableBean接口
					if (autoCloseable) {
						destroyMethodName = CLOSE_METHOD_NAME; // 获取close方法
					}
					else {
						try {
							...
							try {
								destroyMethodName = bean.getClass().getMethod(SHUTDOWN_METHOD_NAME).getName(); // 没有close方法，找shutdown方法
							}
