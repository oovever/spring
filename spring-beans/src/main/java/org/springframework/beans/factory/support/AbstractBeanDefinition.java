...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public String getBeanClassName() {
		...
		if (beanClassObject instanceof Class) { // class类型获取名称
			return ((Class<?>) beanClassObject).getName();
		}
		else {
			return (String) beanClassObject; // string类型直接返回
		}
	}
	 ...
	 */
	public boolean isSynthetic() { // 表示该bean是spring容器内部生成的bean实例,不允许第三方或者用户去改动
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		...
		if (hasMethodOverrides()) { // 判断是否有方法需要重写
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		int count = ClassUtils.getMethodCountForName(getBeanClass(), mo.getMethodName()); // 获取类中方法名个数
		if (count == 0) {
			throw new BeanDefinitionValidationException(
