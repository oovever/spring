...
public class InjectionMetadata {
	...
	public void inject(Object target, @Nullable String beanName, @Nullable PropertyValues pvs) throws Throwable {
		...
		if (!elementsToIterate.isEmpty()) {
			for (InjectedElement element : elementsToIterate) {
				element.inject(target, beanName, pvs); // resource 注入逻辑 遍历注入点执行注入，区分字段、方法
			}
		}
	}
	...
	public abstract static class InjectedElement {
		...
		protected void inject(Object target, @Nullable String requestingBeanName, @Nullable PropertyValues pvs)
			...
			if (this.isField) { // 如果当前字段是属性
				Field field = (Field) this.member;
				ReflectionUtils.makeAccessible(field);
				field.set(target, getResourceToInject(target, requestingBeanName)); // 注入字段
			}
			else {
				if (checkPropertySkipping(pvs)) { // pvs中是否已经有值
					return;
				}
				try {
					Method method = (Method) this.member;
					ReflectionUtils.makeAccessible(method);
					method.invoke(target, getResourceToInject(target, requestingBeanName)); // 注入方法
				}
