...
public class CglibSubclassingInstantiationStrategy extends SimpleInstantiationStrategy {
	...
	private static class CglibSubclassCreator {
		...
		public Object instantiate(@Nullable Constructor<?> ctor, Object... args) {
			...
			Object instance;
			if (ctor == null) {
				instance = BeanUtils.instantiateClass(subclass); // 获取代理对象
			}
			else {
				try {
			...
			factory.setCallbacks(new Callback[] {NoOp.INSTANCE,
					...
					new ReplaceOverrideMethodInterceptor(this.beanDefinition, this.owner)}); // replace-method
			return instance;
		}
	...
	private static class LookupOverrideMethodInterceptor extends CglibIdentitySupport implements MethodInterceptor {
		...
		public Object intercept(Object obj, Method method, Object[] args, MethodProxy mp) throws Throwable {
			...
			LookupOverride lo = (LookupOverride) getBeanDefinition().getMethodOverrides().getOverride(method); // 获取lookup注解信息
			Assert.state(lo != null, "LookupOverride not found");
			Object[] argsToUse = (args.length > 0 ? args : null);  // if no-arg, don't insist on args at all
			if (StringUtils.hasText(lo.getBeanName())) {
				Object bean = (argsToUse != null ? this.owner.getBean(lo.getBeanName(), argsToUse) :
						this.owner.getBean(lo.getBeanName())); // get指定bean
